...
def determine_gl_flags():
    ...
    # DODAJ DEFINICJE MAKR SDL2 + Android NDK system headers dla wszystkich modułów graphics
    base_flags.setdefault('define_macros', []).extend([
        ...
        ('__warnattr_strict(x)', ''),
        ('__mallocfunc', ''),
        ('__BIONIC_ALLOC_SIZE(x)', ''),
        ('__wur', ''),
        ('__RENAME_LDBL(x,y,z)', ''),
        ('__RENAME_IF_FILE_OFFSET64(x)', ''),
        ('__noreturn', ''),
        ('__attribute__(x)', ''),
        ('__returns_twice', ''),
        ('_GNU_SOURCE', '1'),
...
def determine_sdl2():
    ...
    # DODAJ DEFINICJE MAKR SDL2 + Android NDK system headers dla systemu headers konfliktów
    flags['define_macros'] = [
        ('__GNUC_PREREQ(maj,min)', '((__GNUC__ << 16) + __GNUC_MINOR__ >= ((maj) << 16) + (min))'),
        ('__glibc_clang_prereq(maj,min)', '0'),
        ('__glibc_has_attribute(attr)', '0'),
